...
Constraint handling in DE: **simplex repair** for `Σx=1` (implemented in `src/dangote_opt/optimization/objective.py`); quality specs enter as hard filters/penalty terms in Phase 4.
...
Until Phase 3, a documented linear placeholder keeps the optimizer and app testable.

## 4. Baseline contract (locked, spec §3.4)

The optimizer's margin is **always** reported alongside, on identical prices/constraints:
1. **Equal-weight** blend at best severity,
