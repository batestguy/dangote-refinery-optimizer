...
| 2 | EIA Open Data v2 | product prices (gasoline/diesel/jet), refinery utilization, crack-spread proxies | REST, free key (`.env`, `EIA_API_KEY`) | public | 🟡 client implemented + offline-tested 2026-09-19 (`src/dangote_opt/data/acquire.py`); live pull pending key. Route/param contract verified against the official v2 guide (read 2026-09-19): `GET api.eia.gov/v2/{route}/data`, `api_key` in URL, `frequency`, `data[]`, `facets[name][]`, `start`/`end`, `offset`/`length` ≤ 5,000 rows/page, values as JSON strings. Candidate series in `CATALOG`: `petroleum/cons/psump` (product prices, $/gal), `petroleum/sum/tusandm` (refinery inputs → utilization), `petroleum/move/impcus` (crude imports by country). Each cached to `data/raw/eia/*.parquet` + JSON sidecar (URL without key, params, pull date, rows, API version) | **Verify each route/facet on first live pull** (via `scripts/eia_smoke.py`) and pin the confirmed series IDs here |
