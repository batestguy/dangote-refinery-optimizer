...
| 3 | ExxonMobil / TotalEnergies crude assays | per-crude (API, sulfur, TBP) for the 5-crude slate | public web (vendor PDFs in gitignored `data/raw/assays/`) | public | ✅ **2026-09-19, live-verified** — slate built & validated (`scripts/build_slate.py` → committed `data/derived/slate_phase1.parquet` + sidecar). **Final slate (spec §8 item 1 resolved):** Bonny Light (TE, API 34.9, S 0.15 %, assay 18-Jun-20) · Forcados (TE, API 31.5, S 0.22 %, assay 01-Mar-14) · Qua Iboe (XOM QUAIB24Y, API 37.3, S 0.12 %) · Alaska North Slope (XOM ANS25Y, API 32.3, S 1.04 %) · Thunder Horse (XOM THUN225Y, API 34.5, S 0.76 %) | **Substitution note:** spec's placeholder candidates Arab Light + Urals have **no published open TBP assays** (ExxonMobil library checked 2026-09-19; Saudi Aramco sheets are gated, Urals spot assays absent) → substituted ANS + Thunder Horse per the availability rule in spec §8 item 1. Yield basis per crude recorded in `notes` (TE sheets = wt%, XOM = vol% grid) — Phase 2 bridge must not mix bases |
