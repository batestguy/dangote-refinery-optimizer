...
- **Next up (Phase 2):** FCC correlation-set decision (spec §8 item 2) → implement `features/bridge.py` `tbp_cut_fractions` + `yields_from_assay` against the real slate; `docs/methodology.md` cites every correlation. Cost anchor: EIA `EPCBRENT` + documented Bonny differential (row 9).
