...
#   "scikit-learn",
#   "plotly",
#   "pyodide-http; sys_platform == 'emscripten'",  # noqa: FURB bad-name (unused now; kept as wheel-fetch fallback)
#   "requests; sys_platform != 'emscripten'",
#   "python-dotenv; sys_platform != 'emscripten'",
# ]
#
# [tool.marimo.runtime]
# cache_cells = true
...
Deploys three ways from this one file: HF Spaces (Docker, spec §3.7),
GitHub Pages via the WASM export (`marimo export html-wasm` — Python runs
in the visitor's browser), and locally (`uv run marimo run app/app.py`).
WASM data paths: files live under public/ in the export and load through
pyodide's fetch layer via the open_data shim below.
...
def _():
    ...
    # In the WASM export, install the bundled dangote_opt wheel. The URL must
    # be absolute — micropip resolves relative URLs against the worker origin
    # (not the page origin), which silently corrupts the download.
    if sys.platform == "emscripten":
        print("[wasm] installing dangote_opt wheel ...")
        import micropip

        import marimo as _mo_wasm

        _base = str(_mo_wasm.notebook_location()).rstrip("/")
        await micropip.install(
            f"{_base}/public/wheels/dangote_refinery_optimizer-0.1.0-py3-none-any.whl"
        )
        print("[wasm] dangote_opt wheel installed")
...
def _(mo):
    ...
    # embedded as a base64 data URI so it travels inside app.py — the file
    # that deploys to HF Spaces and the WASM export unchanged — with zero
    # extra requests at cold start.
    import base64 as _base64
    from pathlib import Path as _Path

    def _read_bytes_hero(rel):
        """Read repo file locally; in WASM fetch from the export's public/.
        URL built as string — pathlib would collapse the // in http://."""
        p = _Path(rel)
        if p.exists():
            return p.read_bytes()
        from pyodide.http import open_url as _hero_open_url

        base = str(mo.notebook_location()).rstrip("/")
        return _hero_open_url(f"{base}/public/{rel}").read()

    hero_b64 = _base64.b64encode(
        _read_bytes_hero("docs/assets/credited/refinery_site_hero.jpg")
    ).decode()
    css = """
    <style>
...
def _(mo, np):
    ...
    def _read_bytes_loader(rel):
        """Local read, or HTTP fetch from the export's public/ (WASM).
        URL built as string — pathlib would collapse the // in http://."""
        p = Path(rel)
        if p.exists():
            return p.read_bytes()
        from pyodide.http import open_url as _loader_open_url

        base = str(mo.notebook_location()).rstrip("/")
        return _loader_open_url(f"{base}/public/{rel}").read()

    def _read_text_loader(rel):
        return _read_bytes_loader(rel).decode("utf-8")
    ...
    df = pd.read_parquet(_io.BytesIO(_read_bytes_loader("data/derived/slate_phase1.parquet")))
    ...
    except Exception:  # FileNotFoundError locally; different failure shape in WASM
        cost_sidecar = json.loads(_read_text_loader("data/derived/costs_phase2.json"))
    ...
    # petrochem stays on the disclosed CONFIG placeholder. In WASM the live
    # pull is skipped entirely (no .env) — straight to the committed artifact.
    try:
        price_frame = usgc_prices_frame("")
        PRICES = product_prices(price_frame)
    except Exception:  # noqa: BLE001 — offline fallback to the committed artifact
        price_sidecar = json.loads(_read_text_loader("data/derived/prices_phase2.json"))
    ...
    SC = json.loads(_read_text_loader("data/derived/scenarios_phase5.json"))
    SENS = json.loads(_read_text_loader("data/derived/sensitivity_phase4.json"))
...
def _(mo):
    ...
    # In WASM the live fetch is skipped (browser CORS + no key) — snapshot.
    import json as _tjson
    from pathlib import Path as _tpath

    _tsnap_rel = "data/derived/ticker_latest.json"
    if _tpath(_tsnap_rel).exists():
        _snapshot = _tjson.loads(_tpath(_tsnap_rel).read_text(encoding="utf-8"))
    else:
        from pyodide.http import open_url as _ticker_open_url

        _base = str(mo.notebook_location()).rstrip("/")
        _snapshot = _tjson.loads(_ticker_open_url(f"{_base}/public/{_tsnap_rel}").read())
...
def _(SC, mo):  # 03 · Charts — light brand plotly template (navy/red).
    ...
    def _dp(rel):
        p = _cPath(rel)
        return p if p.exists() else mo.notebook_location() / "public" / rel

    _cdu_b64 = _cbase64.b64encode(
        _cPath(_dp("docs/assets/credited/cdu_unit.jpg")).read_bytes()
    ).decode()
    _p1_b64 = _cbase64.b64encode(
        _cPath(_dp("docs/assets/credited/procedures_a.jpg")).read_bytes()
    ).decode()
    _p2_b64 = _cbase64.b64encode(
        _cPath(_dp("docs/assets/credited/procedures_b.jpg")).read_bytes()
    ).decode()
    _p3_b64 = _cbase64.b64encode(
        _cPath(_dp("docs/assets/credited/procedures_c.jpg")).read_bytes()
    ).decode()
